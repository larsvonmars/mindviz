...
const createToolButton = (icon, label, handler, options) => {
    const btn = (0, styles_1.createButton)('secondary', options);
    btn.innerHTML = icon;
...
function createToolbar(vmm) {
    ...
    const layoutBtn = createToolButton(vmm['currentLayout'] === 'radial' ? treeLayoutIcon : radialLayoutIcon, 'Toggle layout', () => {
    ...
    const toggleBtn = (0, styles_1.createButton)('primary', { disableHoverEffect: true });
    ...
    function updateButtonActiveState(button, isActive) {
        ...
        if (svg) {
            svg.style.stroke = styles_1.CSS_VARS.text;
    ...
    function openFileModal() {
        ...
        const btnConfig = [
            { label: "Export as SVG", action: () => { vmm.exportAsSVG(); }, icon: "📄", color: styles_1.CSS_VARS.success },
            { label: "Copy JSON", action: () => {
                ...
                }, icon: "📋", color: styles_1.CSS_VARS.primary
            },
            { label: "Clear All", action: () => {
                    vmm['mindMap'].root.children = [];
                    vmm.render();
                }, icon: "🗑️", color: styles_1.CSS_VARS.danger
            },
            { label: "Import JSON", action: async () => {
                ...
                }, icon: "📥", color: styles_1.CSS_VARS.accent
            },
            { label: "Undo", action: () => { vmm.undo(); }, icon: "↶", color: "#ea580c" },
            { label: "Redo", action: () => { vmm.redo(); }, icon: "↷", color: styles_1.CSS_VARS.primary }
        ];
        btnConfig.forEach((cfg, index) => {
            ...
            btn.addEventListener("mouseenter", () => {
                btn.style.background = styles_1.CSS_VARS.background;
        ...
        closeBtn.addEventListener("mouseenter", () => {
            closeBtn.style.background = styles_1.CSS_VARS.primaryHover;
        ...
        closeBtn.addEventListener("mouseleave", () => {
            closeBtn.style.background = styles_1.CSS_VARS.primary;
