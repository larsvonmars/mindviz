...
export function createWhiteboardToolbar(wb: VisualWhiteboard): HTMLElement {
  ...
  tools.forEach(tool => {
    ...
    if (tool.name === 'select') btn.classList.add('active');
  ...
  const actionTools = [
    ...
    },
    {
      ...
      title: 'Load JSON',
      action: async () => {
        const json = await showInputModal('Load Whiteboard', 'Paste JSON data:', '');
        if (json) {
          try {
    ...
    },
    {
      ...
      action: () => {
        ...
        navigator.clipboard.writeText(json).then(() => {
          ...
          const btn = toolbar.querySelector('[title="Copy JSON"]') as HTMLElement;
...
function createToolButton(icon: string, title: string, action: () => void): HTMLButtonElement {
