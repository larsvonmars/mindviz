...
export function createContextMenu(wb: VisualWhiteboard): HTMLDivElement {
  ...
  function editItem() {
    ...
    if (item.type === 'text') {
      ...
      showInputModal('Edit Image', 'Image URL:', item.content || '').then(newUrl => {
        if (newUrl !== null) {
  ...
  function duplicateItem() {
    ...
    const newItem = { ...item };
    delete (newItem as any).id; // Remove id so board will assign new one
    newItem.x += 20;
    newItem.y += 20;
  ...
  wb.container.addEventListener("contextmenu", e => {
    ...
    if (clickedItem) {
      ...
      const deleteBtn = menu.querySelector("button:last-child") as HTMLElement;
      if (currentItems.length > 1) {
