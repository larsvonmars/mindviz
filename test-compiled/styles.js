...
export const createBaseElement = (tag, styles) => {
    const el = document.createElement(tag);
    Object.assign(el.style, styles);
    return el;
};
export const createInput = (type = 'text') => {
    const input = createBaseElement('input', {
        width: '100%',
...
export const createButton = (variant = 'secondary', options) => {
    const { disableHoverEffect = false } = options || {};
    const button = createBaseElement('button', {
...
export const extractSolidColor = (bg) => {
...
export const createLoadingSpinner = (size = 24) => {
    const spinner = createBaseElement('div', {
        width: `${size}px`,
        height: `${size}px`,
...
export const createCloseIcon = (onClose) => {
    const closeIcon = createBaseElement('div', {
        cursor: 'pointer',
        opacity: '0.7',
