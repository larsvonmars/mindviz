...
export function createMindNodeElement(options) {
    ...
    const nodeDiv = createBaseElement('div', {
        position: 'absolute',
        left: `${x}px`,
        top: `${y}px`,
        ...
        zIndex: '1',
        background: mindNode.background || CSS_VARS['node-bg'],
    ...
    if (mindNode.description) {
        const toggleButton = createBaseElement('div', {
            cursor: 'pointer',
    ...
    if (options.mindNode.imageUrl) {
        const imgContainer = createBaseElement('div', {
